...
const statusConfig = {
  active: { label: '运行中', bg: 'rgba(34,197,94,0.12)', color: '#22c55e', icon: CheckCircle2 },
  standby: { label: '待命', bg: 'rgba(245,158,11,0.12)', color: '#f59e0b', icon: AlertCircle },
}
    ...
    return [
      {
        ...
        detail: chatModel?.base_url || '来自设置页偏好',
      },
      {
        label: '融合模型',
        ...
        detail: `${analysisModel?.base_url || '偏好'} + ${reviewModel?.base_url || '偏好'}`,
      },
      SAMPLE_MODELS[2],
    ]
  }, [available, prefs])
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-base font-bold mb-1 text-[var(--text-primary)]">模型配置</h2>
          <p className="text-xs text-[var(--text-muted)]">大语言模型与检索引擎参数</p>
        </div>
        <button
