...
对于每个词汇，请分类为以下类别之一：
- real_name: 真实的中国人名（如"用户A","用户B","用户C","用户D","用户E"）
- pronoun: 代词或称谓（如"朋友"、"同学"、"老师"、"阿姨"、"哥哥"）
- animal: 动物名或宠物名（如"猫咪"、"小狗"、"小白兔"）
- common: 常见词汇、地名、公司名等非人名（如"CITY_A"、"公司"、"时间"、"DISTRICT_A"、"DISTRICT_B"）
...
2. 只有独立的人名才归类为 real_name，例如：
   - "用户A" → real_name（独立人名）
   - "用户B" → real_name（独立人名）
   - "用户C" → real_name（昵称形式的人名）
3. 以下情况不是人名，应归类为 common：
   - 以"和"开头的词组（如"和老师","和用户A","和他们"）→ common
   - 以"有"开头的词组（如"有老师"）→ common
   - 地名（如"CITY_A"、"CITY_B"、"DISTRICT_A"、"DISTRICT_B"、"LANDMARK_A"、"LANDMARK_B"）→ common
