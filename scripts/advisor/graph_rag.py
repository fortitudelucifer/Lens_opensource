...
class RetrievalResult:
    ...
    def __contains__(self, key: str) -> bool:
        return key in {'conversation_id', 'conversation_text', 'timestamp', 'score', 'metadata', 'text'}
...
def _create_faiss_index(dim: int, n_vectors: int, index_type: str = "auto"):
    """
    FAISS 索引工厂：
      - "auto": N < IVF_THRESHOLD → FlatIP，否则 → IVFFlat
      - "flat": 强制 FlatIP（暴力搜索，100% 召回率）
      - "ivf": 强制 IVFFlat（nlist=√N, nprobe=10）
      - "hnsw": 预留接口（未来迁移到 Qdrant/pgvector，当前 fallback 到 FlatIP）
    """
    import faiss
    import math

    if index_type == "hnsw":
        logger.warning("[FAISS] HNSW 尚未实现，fallback 到 FlatIP。未来将迁移到 Qdrant。")
        return faiss.IndexFlatIP(dim)

    if index_type == "flat" or (index_type == "auto" and n_vectors < IVF_THRESHOLD):
        logger.info(f"[FAISS] 使用 FlatIP（{n_vectors} 向量）")
        return faiss.IndexFlatIP(dim)
    ...
    index.nprobe = 10
    logger.info(f"[FAISS] 使用 IVFFlat（{n_vectors} 向量，nlist={nlist}，nprobe=10）")
    return index
...
class GraphRAGManager:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self.embedding_model_name = self._resolve_model_path(
            config.get('embedding_model', '/data/models/bge-m3')
        )
        self.reranker_model_name = self._resolve_model_path(
            config.get('reranker_model', '/data/models/bge-reranker-v2-m3')
        )
    ...
    def _resolve_model_path(model_name: str) -> str:
        ...
        local_path = Path('/data/models') / model_name.split('/')[-1]
    ...
    def _segment_to_documents(self, messages: list[dict]) -> list[dict]:
        docs = []
        window_size = self.config.get('document_window_size', 10)
        step_size = self.config.get('document_step_size', window_size)
        for i in range(0, len(messages), step_size):
            ...
            for msg in window:
                ...
                text = msg.get('text_raw') or msg.get('conversation_text') or msg.get('text') or ''
                if text:
            ...
            docs.append({
                'doc_id': f"doc_{i:06d}",
                'text': '\n'.join(lines),
                'start_ts': window[0].get('ts') or window[0].get('timestamp', ''),
                'end_ts': window[-1].get('ts') or window[-1].get('timestamp', ''),
    ...
    def build_index(self, conversations: list[dict], show_progress: bool = True):
        ...
        _idx_type = self.config.get('index_type', 'auto') if hasattr(self, 'config') else 'auto'
        self._faiss_index = _create_faiss_index(self._embedding_dim, len(embeddings), index_type=_idx_type)
    ...
    def update_index(self, new_conversations: list[dict], show_progress: bool = True):
        ...
        if new_conversations and any('text_raw' in item for item in new_conversations):
            new_docs = self._segment_to_documents(new_conversations)
            if not new_docs:
                return
            ...
            self._conversations = [
                {
                    'conversation_id': doc.get('doc_id', f'doc_{i}'),
                    'conversation_text': doc.get('text', ''),
                    'timestamp': doc.get('start_ts', ''),
                    'metadata': doc,
                }
        ...
        if self._faiss_index is None:
            ...
            self._faiss_index = _create_faiss_index(self._embedding_dim, len(new_embeddings))
    ...
    def save_index(self, path: Optional[str] = None):
        ...
        faiss.write_index(self._faiss_index, str(tmp_path))
    ...
    def _vector_search(
        ...
        if self._faiss_index is None and self._documents and self._embeddings is not None:
            self._load_embedding_model()
            ...
            for idx in order:
                ...
                results.append(RetrievalResult(
                    conversation_id=doc.get('doc_id', f'doc_{int(idx)}'),
                    conversation_text=doc.get('text', ''),
                    timestamp=doc.get('start_ts', ''),
