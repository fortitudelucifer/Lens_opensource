...
| GLM-4.7 Response API 特殊性 | 四轮迭代修复，单轮格式稳定 |
...
所有 prompt 均包含"如果有历史对话上下文"引导语，确保 RAG 注入时 Agent 正确引用检索到的历史对话。
...
| **DeepSeek/GLM/Qwen/Kimi/Doubao（备用）** | `/v1/chat/completions` | 标准多轮，max_tokens ≤ 16384 |
| **本地 Ollama** | `/v1/chat/completions` | qwen3:8b，无 token 限制 |

### 3.2 GLM-4.7 四轮修复历程

GLM-4.7 的 Response API 与标准 Chat Completions API 差异巨大，经历了四轮迭代才稳定：
...
| 层级 | 机制 | 持久化 | 触发条件 | 说明 |
|------|------|--------|----------|------|
| **L1 滑动窗口** | 保留最近 20 条完整消息 | session JSON | 消息数 > 20 | 超出部分进入 L2 |
| **L2 历史摘要** | 旧消息压缩为要点列表 | session JSON | 有旧消息溢出 | 注入 system prompt `【历史摘要】` |
| **L3 关键事实** | 自动提取日期事件 · 关系状态 | `session.memory_facts` | 每轮对话后 | 跨轮次累积（上限 30 条），注入 `【记忆事实】` |
...
| 后端 | Thinking 格式 | 提取方式 |
|------|-------------|----------|
| DeepSeek | `reasoning_content` 字段 | 直接读取 |
| Qwen | `reasoning_content` 字段 | 直接读取 |
| DeepSeek | `reasoning_content` 字段 | 直接读取 |
| Qwen3 (本地) | `<think>...</think>` 标签 | 正则提取 |
...
| 端点 | 方法 | 功能 | 请求体 |
|------|------|------|--------|
| `/api/chat` | POST | 流式对话（SSE） | `{messages, backend, agent_type, mode}` |
...
conda run -n CHAT_APP_DHA uvicorn scripts.advisor.api.server:app --reload --port 8787 --host 0.0.0.0
