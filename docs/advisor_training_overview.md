...
| 【多模态信号】 | `multimodal_signals` | Kimi 专家 |
| 【修复尝试】 | `repair_attempts` | MoA 分析 |
| 【人格动态】 | `personality_dynamics` | DeepSeek 专家 |
...
- 独立 conda 环境 `CHAT_APP_DHA_unsloth`（Unsloth 与 HF transformers 版本冲突）
...
```bash
# 1. 反匿名化（含分析文本）
conda run -n CHAT_APP_DHA python scripts/advisor/run_all/_05c_deanonymize_training.py \
...
# 2. 过滤 + 分割
conda run -n CHAT_APP_DHA python scripts/advisor/run_all/_05b_filter_split_training.py \
