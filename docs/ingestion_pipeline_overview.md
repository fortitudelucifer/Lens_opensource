...
CHAT_APP_DHA 流水线原本只支持CHAT_APP HTML+CSV 导出格式。随着需求扩展到 Telegram、WhatsApp 等多种即时通讯平台，需要一个**统一归一化入口**，将异构数据源转换为标准格式后进入下游处理流水线。
...
graph TB
    subgraph "输入层：多源数据"
        S1[CHAT_APP HTML+CSV<br/>CHAT_APP Desktop 导出]
    ...
    subgraph "适配器层：Adapter Registry"
        ...
        A1[CHAT_APPAdapter<br/>复用 extract_html_to_jsonl.py]
...
└── ingestion/
    ...
    └── adapters/
        ...
        ├── CHAT_APP_html.py      # CHAT_APP HTML+CSV
...
|--------|-------------|----------|-------------|----------|
| **CHAT_APPAdapter** | `CHAT_APP_html` | HTML + CSV | `P1:` | 复用 `extract_html_to_jsonl.py` 核心逻辑 |
...
### 4.3 CHAT_APP适配器 (CHAT_APP_html)

**核心脚本**: `adapters/CHAT_APP_html.py`
...
| `test_CHAT_APP_adapter.py` | HTML 解析、CSV 元数据、类型映射 |
