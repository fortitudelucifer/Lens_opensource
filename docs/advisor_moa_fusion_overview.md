...
│  2. 有机融合：Qwen 基于原始对话和多方分析重写，而非机械拼接      │
...
graph TB
    subgraph "S1: 三专家并行分析"
        A1[DeepSeek V3.2<br/>深度心理分析]
        A2[GLM-4.7<br/>批判性审视]
        A3[Kimi 2.5<br/>多模态信号分析<br/>条件触发: mm_density ≥ 3]
        A1F[DeepSeek V3.2-Speciale<br/>S1 降级备选]
        A1FF[GLM+Kimi 双分析<br/>S1 最终回退]
    end

    subgraph "S2: MoA 有机融合"
        B1[Qwen3<br/>有机融合重写]
        B2[Qwen3 备用 key<br/>S2 降级 #1]
        B3[Kimi K2.5<br/>S2 降级 #2]
        B4[v1 程序合并<br/>S2 最终回退]
    end

    subgraph "S3: 审核评分"
        C1[Qwen 4.1<br/>5维10分制评分]
        C1F[Qwen 备用 key<br/>S3 降级 #1]
    ...
    subgraph "S4: 定向补齐"
        D1[低分维度检测<br/>≤7分触发补齐]
        D2[Qwen 定向补齐<br/>最多3轮迭代]
        D2F[Qwen 备用 key<br/>S4 降级 #1]
        D2FF[Kimi 3 Pro<br/>S4 降级 #2]
...
|------|------|-------------|----------|------|
| **S1 多专家分析** | DeepSeek V3.2 + GLM-4.7 + Kimi K2.5 | 第三方 API 代理（3 个不同 key pool） | `ThreadPoolExecutor(3)` 真并行 | 不同 API key 避免共享限流 |
| **S2 有机融合** | Qwen3 | 第三方 API 代理 | 顺序（依赖 S1） | MoA prompt 指导"有机融合，非简单拼接" |
| **S3 审核评分** | Qwen3 | 第三方 API 代理 | 顺序 | 5 维 10 分制（准确性/深度/多模态/结构/流畅度），满分 50 |
| **S4 维度补齐** | Qwen3 | 第三方 API 代理 | 按需（仅 ≤7 分维度） | 最多 3 轮迭代，每轮只补齐低分维度 |
...
| 专家 | 模型 | 角色 | 分析重点 | 触发条件 |
|------|------|------|----------|----------|
| **DeepSeek** | V3.2 | 主分析师 | 依附风格、人格动态、深层心理洞察 | 全部 chunk |
| **GLM** | 4.7 | 独立审视 | 批判性审查、权力动态、沟通模式 | 全部 chunk |
| **Kimi** | K2.5 | 多模态专家 | 语音情绪、表情包含义、图片上下文 | `mm_density ≥ 3` 时触发 |
...
| 场景 | 处理策略 | 融合质量标记 |
|------|----------|-------------|
| DeepSeek + GLM 均成功 | Qwen 有机融合重写 | `moa_full` |
| 仅 DeepSeek 成功 | 直接使用 DeepSeek 结果 | `partial` |
| 仅 GLM 成功 | 直接使用 GLM 结果 | `partial` |
| 均失败 | 返回空 | `failed` |

**MoA 聚合器 Prompt 要点**:
- 要求 Qwen "有机融合"而非"简单拼接"
- 提供原始对话前 4000 字作为交叉验证
- Kimi 多模态分析作为可选补充节
- 输出格式与上游 13 字段 schema 完全对齐

**Fallback 链**: `Qwen(主线) → Qwen(备用 key) → Kimi → v1 程序合并`
...
借鉴 CPU 指令流水线思想：S1（Analysis）使用 DeepSeek/GLM/Kimi API，S2-S4（MoA/Review/Remediation）使用 Qwen API。两组 API 天然不竞争，因此当 chunk[i] 进入 S2 时，chunk[i+1] 可立即开始 S1。
...
class PipelineExecutor:
    ...
    def __init__(self, generators, agent_type, ...):
        ...
        self._Qwen_sem = asyncio.Semaphore(max_concurrent_Qwen)  # S2-S4 并发度（默认 3）
...
| **Qwen Semaphore** | `asyncio.Semaphore(3)` | S2/S3/S4 最多 3 个并发 Qwen 请求 |
...
Qwen 审核 verdict 偏严（总分 44 仍判 `needs_revision`），因此引入分数覆盖：

```python
# _02c_fusion_pipeline.py, run_Qwen_review()
...
|---------|---------|------|
| ≥ 44 | **pass** (覆盖) | 高分强制通过 |
| 36-43 | 依赖模型判断 | Qwen 原始 verdict |
| 20-35 | needs_revision | 需要补齐 |
| < 20 | fail | 严重质量问题 |

### 5.3 定向补齐机制

**核心函数**: `run_Qwen_remediation()`

```python
def run_Qwen_remediation(Qwen_gen, fused_features, review_scores, conversation, ...):
    """
    定向补齐: 单项 ≤ REMEDIATION_THRESHOLD(7) 的维度
    Fallback 链: Qwen(主线) → Qwen(备用 key) → Kimi → Kimi
    最多 3 轮迭代
    """
...
graph TB
    subgraph "S1 Analysis 降级"
        CA[DeepSeek V3.2] -->|失败| CB[DeepSeek V3.2-Speciale]
        CB -->|失败| CC[GLM+Kimi 双分析]
    end

    subgraph "S2 MoA 融合降级"
        GA[Qwen 4.1 主线] -->|失败| GB[Qwen 4.1 备用 key]
        GB -->|失败| GC[Kimi K2.5]
        GC -->|失败| GD[v1 程序合并]
    end

    subgraph "S3 审核降级"
        RA[Qwen 4.1 主线] -->|失败| RB[Qwen 4.1 备用]
        RB -->|失败| RC[Kimi K2.5]
    end

    subgraph "S4 补齐降级"
        DA[Qwen 主线] -->|失败| DB[Qwen 备用]
        DB -->|失败| DC[Kimi]
        DC -->|失败| DD[Kimi]
    end
...
| MoA 完整融合 | ~97% |
| MoA 回退到 v1 | ~0.6% |
| DeepSeek 降级到 Sonnet | ~1.6% |
| DeepSeek 降级到 GLM+Kimi | ~0.4% |
| 补齐触发 | ~31% |
| 首次审核通过率 | ≥ 85% |
| 补齐后最终通过率 | ≥ 97% |
...
| `moa_merge_analyses()` | Qwen_gen, DeepSeek/GLM/Kimi_result, ... | dict | MoA 有机融合 |
| `merge_analyses()` | DeepSeek/GLM/Kimi_result | dict | v1 程序合并（回退） |
| `run_Qwen_review()` | gen, conversation, analysis_text, ... | dict | 五维审核评分 |
| `run_Qwen_remediation()` | Qwen_gen, features, scores, ... | (dict, int) | 定向补齐循环 |
