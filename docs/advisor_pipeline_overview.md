...
│                                                    │               │
│  ┌─────────────────── LLM 后端 ──────────────────┐ │               │
│  │ Cloud: GLM-4.7 | DeepSeek V3.2 | Kimi K2.5 | Qwen3 │◄┘               │
│  │        DeepSeek V3.2 | Qwen3 | GLM-4.7    │                │
...
用户输入 → ChatPanel → POST /api/chat
  ...
  → 合并连续同角色消息 (防止 DeepSeek/OpenAI 400)
...
- **MoA 架构**: DeepSeek(S1) + GLM(S1) + Kimi(S1) 并行分析 → Qwen MoA 融合(S2) → Qwen 审核(S3) → Qwen 补齐(S4)
...
- **降级策略**: DeepSeek + GLM + Qwen + Kimi + Doubao（备用） → v1 程序合并
- 断点续传、SchemaValidator 2 轮自修复、rich 实时可视化

### Phase 3: 质量审核 + 自动补齐
- **3a AI 审核 (Qwen Review)**: 5 维度 50 分制（准确/深度/平衡/安全/结构，各 1-10）
...
- **API 兼容层**:
  - Response API (GLM-4.7): 单轮 system+user input（第三方代理 不支持多轮/instructions）
  - Chat Completions (DeepSeek/Kimi/Qwen 等): max_tokens ≤ 16384
...
| 模型连通 | 9/9 后端 | 9/9 ✅ (GLM-4.5 Response API 单轮适配) |
