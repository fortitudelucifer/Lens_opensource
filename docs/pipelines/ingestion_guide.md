...
本文档说明如何使用归一化输入接口，将不同来源的聊天记录导入 wechatDHA 项目。

归一化输入接口支持 5 种数据来源：

| 来源类型 | 标识 | 输入格式 |
|---------|------|---------|
| 微信 | `wechat_html` | HTML + CSV 导出文件 |
...
# 1. 将原始素材文件夹放到项目目录下
cp -r ~/new_chat <WORKSPACE_ROOT>        # ← 替换 new_chat 为你的文件夹名

# 2. 从模板工作空间复制 scripts（init_workspace.py 在 scripts/ 里，必须先有）
cp -r <WORKSPACE_ROOT>/scripts <WORKSPACE_ROOT>/scripts  # ← 同上

# 3. 运行初始化（自动检测来源类型）
cd <WORKSPACE_ROOT>                       # ← 同上
...
# 或指定来源类型
python scripts/workspace/init_workspace.py --source-type wechat_html --contact-name "联系人名"
```
