...
Linkfile 流水线处理微信聊天记录中 `type=49` 的消息，这类消息包含多种子类型：
...
```json
{
  ...
  "sub_type": 57,
  "link_sub_type": "quote",
  "quote_svrid": "1818135530269614685",
  "quote_type": 1,
...
Linkfile 流水线是 wechatDHA 项目中最轻量但功能最丰富的模态处理流水线：
...
通过这个流水线，可以将微信聊天中的引用、链接、文件、小程序等多种消息类型统一处理，为后续的数据分析和 LLM RAG 提供结构化的数据支持。
