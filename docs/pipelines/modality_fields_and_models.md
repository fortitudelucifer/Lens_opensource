...
- [20. 安全伦理与四级危机干预](#20-安全伦理与四级危机干预)
  - [20.1 四级检测流水线](#201-四级检测流水线)
  - [20.2 第一人称前置检测](#202-第一人称前置检测)
  - [20.3-20.4 上下文累积 · 误触发排除](#203-上下文累积感知)
  - [20.5 用词红线](#205-用词红线双重拦截)
  - [20.6-20.7 分级资源体系 · RED 模板](#206-分级资源体系)
  - [20.8-20.9 知情同意 · Arena 安全治理](#208-知情同意)
  - [20.10 核心文件索引](#2010-核心文件索引)
- [21. Arena 双镜对比与 Elo 排名](#21-arena-双镜对比与-elo-排名)
  - [21.1 核心交互流程](#211-核心交互流程)
  - [21.2 功能矩阵](#212-功能矩阵含自动跳转与重命名删除)
  - [21.3 API 端点](#213-api-端点已补齐-putdelete)
  - [21.4 Bradley-Terry Elo 算法](#214-bradley-terry-elo-算法)
  - [21.5 五种顾问流派](#215-五种顾问流派)
  - [21.6 五维评分体系](#216-五维评分体系)
  - [21.7 数据结构与持久化](#217-数据结构与持久化)
  - [21.8 高级分析](#218-高级分析)
  - [21.9 前端组件](#219-前端组件sessionoptions-接入)
...
graph TB
    subgraph "Phase -1: 归一化输入"
        Z1["多源数据<br/>WeChat HTML · Telegram JSON<br/>WhatsApp TXT · CSV · JSONL"]
    ...
    subgraph "Phase 8: MoA 多专家融合"
        K1[Claude+GPT+Gemini<br/>三专家并行分析]
        K2[Grok 有机融合<br/>审核+补齐]
    end
...
|-------------|--------|----------|-------------|------|
| `wechat_html` | WeChatAdapter | HTML + CSV | `P1:` | 复用 `extract_html_to_jsonl.py` 核心逻辑 |
...
| 模型名称 | 角色 | 使用阶段 | 说明 |
|----------|------|----------|------|
| **Claude Opus 4.6 Think** | MoA S1 专家 | Phase 8: 融合分析 | 深度心理分析，思考链推理 |
| **GPT-5.2 xhigh** | MoA S1 专家 | Phase 8: 融合分析 | 批判性审查，Response API |
| **Gemini 3 Pro** | MoA S1 专家（条件触发） | Phase 8: 融合分析 | 多模态信号分析，≥3 markers 时触发 |
| **Grok 4.1 Thinking** | MoA S2 融合 + 审核 | Phase 8: 融合 + 审核 | 有机融合 + 5 维评分 + 补齐 |
| **Kimi K2.5** | 审核备选 | Phase 8: 审核降级 | Grok 不可用时的审核替代 |
...
| `link_type` | 规则提取 | string | 固定 | 链接分类（wechat_article / map_location / web_link 等） |
...
| URL 模式 | 分类 | 说明 |
|---------|------|------|
| mp.weixin.qq.com | wechat_article | 微信公众号文章 |
...
graph TB
    ...
    subgraph "Phase A: 对话提取 + MoA 分析"
        ...
        D[_02c_fusion_pipeline.py<br/>Claude+GPT+Gemini → Grok MoA]
...
**创新点**: 每个 chunk 同时输出 `mm_density` 元数据，记录 6 类多模态信号（语音/图片/表情包/视频/位置/情绪标记）的出现计数和占比，下游融合分析据此决定是否触发 Gemini 多模态分析专家。
...
```mermaid
graph LR
    A[Chunk<br/>含 mm_density] --> B1[Claude Opus 4.6 Think<br/>深度心理分析<br/>sk-1ave key pool]
    A --> B2[GPT-5.2 xhigh<br/>批判性审视<br/>sk-4r2m key pool]
    A --> B3[Gemini 3 Pro<br/>多模态信号解读<br/>sk-Ii21 key pool<br/>仅 mm_density ≥3 时触发]

    B1 --> C[Grok 4.1 Thinking<br/>MoA 有机融合<br/>非简单拼接]
    B2 --> C
    B3 --> C

    C --> D[Grok Review<br/>5 维 10 分制<br/>满分 50]
    D --> E{任意维度 ≤7?}
    E -->|是| F[Grok Remediation<br/>按维度补齐<br/>最多 3 轮]
...
| -------------------| ------------------------------------------------------| ------------------------------------| --------------------------------| ---------------------------------------------------------------|
| **S1 多专家分析** | Claude Opus 4.6 Think + GPT-5.2 xhigh + Gemini 3 Pro | 第三方api代理（3 个不同 key pool） | `ThreadPoolExecutor(3)` 真并行 | 不同 API key 避免共享限流　　　　　　　　　　　　　　　　　　 |
| **S2 有机融合**　 | Grok 4.1 Thinking　　　　　　　　　　　　　　　　　　| 第三方api代理　　　　　　　　　　　| 顺序（依赖 S1）　　　　　　　　| MoA prompt 指导"有机融合，非简单拼接"　　　　　　　　　　　　 |
| **S3 审核评分**　 | Grok 4.1　　　　　　　　　　　　　　　　　　　　　　 | 第三方api代理　　　　　　　　　　　| 顺序　　　　　　　　　　　　　 | 5 维 10 分制（心理深度/实用性/平衡性/具体性/共情性），满分 50 |
| **S4 维度补齐**　 | Grok 4.1　　　　　　　　　　　　　　　　　　　　　　 | 第三方api代理　　　　　　　　　　　| 按需（仅 ≤7 分维度）　　　　　 | 最多 3 轮迭代，每轮只补齐低分维度　　　　　　　　　　　　　　 |

#### 三专家角色分工

| 专家 | 模型 | 分析维度 | 触发条件 |
|------|------|----------|----------|
| **心理分析师** | Claude Opus 4.6 Think | 依附风格、情感动态、防御机制、权力结构 | 必触发 |
| **批判审视者** | GPT-5.2 (xhigh reasoning) | 沟通模式、NVC 违反、认知偏差、改善方案 | 必触发 |
| **多模态解读者** | Gemini 3 Pro | 语音语气变化、表情包情绪、回复时间模式 | 仅当 chunk 的 `mm_density` ≥ 3 个多模态标记时触发 |

#### S2 融合策略

Grok MoA 融合不是简单拼接三位专家输出，而是通过专用 prompt 引导执行**有机整合**：
- 以 Claude 深度心理分析为**骨架**
- 用 GPT 批判性视角**补充和质疑**
- 将 Gemini 多模态信号**织入对应分析段落**
...
| 吞吐瓶颈 | GPT-5.2 xhigh reasoning |
| 全量 chunks 总耗时 | ~2 小时 |
| S1 并行加速比 | ~2.5x（vs 串行） |
| Gemini 触发率 | 4/10（仅多模态密集 chunk） |
...
graph LR
    subgraph "Stage 1: 分析"
        A1[Chunk N] --> A2[Claude+GPT+Gemini<br/>ThreadPoolExecutor]
    end
    subgraph "Stage 2: 融合"
        B1[Chunk N-1] --> B2[Grok MoA<br/>有机融合]
    end
    subgraph "Stage 3: 审核"
        C1[Chunk N-2] --> C2[Grok Review<br/>5维评分]
    end
    subgraph "Stage 4: 补齐"
        D1[Chunk N-3] --> D2[Grok Remediation<br/>≤7分维度]
    end
...
**关键约束**: OpenAI-compatible proxy 代理平台对单账户有全局 RPM 限制（实测 ~20 RPM），超限触发 429 或临时封禁。`GlobalRateLimiter` 在所有 `KeyRotator` 之上实施硬限制。
...
每条融合分析由 Grok 进行独立审核，按 5 个维度各 10 分制评分：
...
**判定规则**:
- 总分 ≥ 44 → 自动 `pass`（verdict override 机制，解决 Grok 过严问题）
- 任意维度 ≤ 7 → 触发该维度的定向补齐（Remediation）
- 补齐最多 3 轮迭代

### 12.2 Verdict Override 机制

**问题发现**: Grok 4.1 Thinking 作为审核者过于严格，大量高质量分析（总分 44-48）被标记为 `needs_revision`。抽样分析发现这些"需修改"的分析在人工评估中质量优秀。

**解决方案**: 实施总分覆盖机制：

```python
if total_score >= 44:
    verdict = "pass"  # 覆盖 Grok 的 needs_revision
```
...
**问题**: Grok 4.1 **Thinking** 模型在"思考"阶段消耗大量 token，导致实际 JSON 输出被截断（`max_tokens` 不含思考 token）。
...
**应对策略**: 截断时自动降级到非 Thinking 模型（如 Grok 4.1 非 Thinking），或切换到 Kimi K2.5 备用后端。
...
|------|--------|--------|--------|--------|
| **S1 分析** | Claude Opus 4.6 Think | Claude Sonnet Think | GPT+Gemini 双分析 | — |
| **S2 融合** | Grok 4.1 Thinking | Grok 4.1 (非 Thinking) | Kimi K2.5 | — |
| **S3 审核** | Grok 4.1 | Kimi K2.5 | Gemini 3 Pro | — |
| **S4 补齐** | Grok 4.1 | Grok (backup key) | Kimi K2.5 | 保留 pre-remediation 版本 |
...
- 独立 conda 环境 `wechatDHA_unsloth`（Unsloth 依赖与 HF transformers 有版本冲突）
...
除了用户聊天记录的 RAG 检索外，系统还支持注入循证心理学专业知识：

| 知识类别 | 目录 | 示例条目 |
|----------|------|---------|
| 沟通技巧 | `knowledge/communication/` | NVC 四要素、我信息表达法、积极倾听 |
| 危机应对 | `knowledge/crisis/` | 5-4-3-2-1 接地法、蝴蝶拥抱、4-4-6 呼吸 |
| EFT 资源 | `knowledge/eft_resources/` | 追逃循环、三阶段、依恋需求 |
| 治疗手册 | `knowledge/therapy_manuals/` | WHO PM+、ACT（待补充） |
| 跨学科视角 | `knowledge/perspectives/` | 社会学/哲学/博弈论/文化（Phase II） |

**加载**：启动时 `rglob("*.jsonl")` 递归扫描所有子目录。**检索**：`_search_faq()` 关键词匹配 + keywords 加权 + agent_type 分类加权（如 EFT 顾问优先匹配 eft 类条目）。**注入**：以 `【专业知识】` 标签注入 system prompt。

**三开关独立控制**（前端 ChatTopBar / ArenaPage 顶栏）：

| 开关 | 字段 | 控制内容 | 颜色 |
|------|------|---------|------|
| 聊天记录 | `use_rag` | 用户真实聊天记录 RAG | 绿色 |
| 专业知识 | `use_knowledge` | 循证心理学 FAQ 知识 | 紫色 |
| 测评注入 | `inject_enabled` | PHQ-2/GAD-2/依恋/冲突结果 | 测评页 |

### 15.9 索引类型自动选择

`graph_rag.py` 的 `_create_faiss_index()` 工厂函数根据向量数自动选择索引类型：

| 条件 | 索引类型 | 复杂度 | 说明 |
|------|---------|--------|------|
| N < 2000 | FlatIP | O(N) | 当前默认，500 向量延迟 <50ms |
| N >= 2000 | IVFFlat | O(√N) | nlist=√N, nprobe=10，自动训练 |
| 远期 | HNSW | O(log N) | 预留接口，当前 fallback 到 FlatIP |

`_09_build_graph.py` 支持 `--index-type auto/flat/ivf/hnsw` 参数。`save_index()` 使用原子写入（先 `.new` 再 rename）实现蓝绿部署。
...
| 向量维度 | 1024 |
| 索引类型 | FAISS FlatIP（N<2000）/ IVFFlat（N>=2000，自动切换） |
...
5 种 Agent 类型 × 2 种交互模式 = 10 个独立 System Prompt（`core/prompts.py`），覆盖不同咨询场景：

| Agent | agent_type | 倾听模式（5-7 句） | 咨询模式（800-2000 字） |
|-------|-----------|----------|----------|
| **中立顾问** | `neutral` | 共情 + 开放性问题 | 沟通模式 · 依附风格 · NVC · 权力动态 · 家庭系统 |
| **支持性顾问** | `supportive` | 无条件情感验证 | 用户视角分析 · 保护性建议 · 边界设立 |
| **精神分析顾问** | `psychoanalytic` | 均匀悬浮注意力 · 防御机制识别 | 客体关系 · 拉康三界 · 欲望结构 · 移情 |
| **EFT 情绪聚焦** | `eft` | Tango 9 步 move · 追逃循环 | 循环分析 · 依恋需求映射 · 新互动脚本 |
| **家庭系统顾问** | `bowen` | 三角关系 · 自我分化 · 情绪切断 | 代际传递 · 分化评估 · "假设+证据+验证"格式 |

EFT 顾问特有阶段追踪（`eft_stage`: exploration → comforting → action），前端 ChatTopBar 显示粉色阶段标签。Bowen 顾问自动检测第三方提及并注入三角关系分析指令。

所有 prompt 均包含"如果有历史对话上下文"引导语，确保 RAG 注入时 Agent 正确引用。新增顾问只需修改 3 文件：`core/prompts.py` + `types/index.ts` + `constants.ts`。

### 16.2 API 兼容层（四轮迭代）

不同云端后端的 API 格式差异由统一兼容层处理。GPT-5.2 的 Response API 经历了四轮修复才稳定：
...
| **Thinking 内容** | `reasoning_content` / `<think>` 标签检测 | Claude/Grok/DeepSeek 思考过程分离显示 |
...
# 4. MoA 融合分析
conda run -n wechatDHA python scripts/advisor/run_all/_02c_fusion_pipeline.py --moa
...
| API 适配层 | Claude→Sonnet→GPT; Grok→Kimi | ✅ |
...
> 📖 **详细设计文档**：[safety_crisis_overview.md](safety_crisis_overview.md) — 四级检测流水线、关键词配置详解、用词红线、分级资源体系、Arena 安全治理、测评集成、前端组件、集成点矩阵
...
```
用户消息 → Step1 精确匹配(20词) → Step2 第一人称受限(3词) → Step3 完整短语(9短语)
         → Step4 上下文累积(近3轮) → Step5 误触发排除 → 最终级别
```

| 级别 | 触发词数 | AI 行为 | 前端表现 | 归档 |
|------|----------|---------|----------|------|
| 🟢 GREEN | — | 正常生成 | 标准界面 | 否 |
| 🟡 YELLOW | 22 精确词 | 正常生成 + 安全引导注入 system prompt | 显示自助资源（呼吸/接地/书写/安抚） | 否 |
| 🟠 ORANGE | 22 精确词 + 6 短语 | 正常生成 + 高风险引导注入 | 强制显示热线 + 专业引导 | 否 |
| 🔴 RED | 20 精确词 + 3 第一人称 + 9 短语 | **完全中断 AI** → 固定模板 | 全屏危机干预 | ✅ `crisis_archive/` |

### 20.2 第一人称前置检测

解决"猫想死"类误触发：关键词前 5 字符窗口内必须包含 `我|自己|本人|俺|咱`。

```python
prefix = text[max(0, idx - 5):idx]
if re.search(r"(我|自己|本人|俺|咱)", prefix):
    matched.append(kw)
```
...
单条未达高级别，但连续多轮困扰信号时自动升级：

| 累积条件 | 升级 |
|----------|------|
| 近 3 轮 yellow ≥ 2 | → YELLOW |
| 近 3 轮 yellow ≥ 3 | → ORANGE |
| 近 3 轮 orange ≥ 2 | → RED |

### 20.4 误触发排除

消息匹配 11 个排除模式（"新闻里说" "电影里" "开玩笑" 等）时，级别 -1（RED→ORANGE, ORANGE→YELLOW）。

### 20.5 用词红线（双重拦截）

| 层级 | 机制 | 禁用词数 |
|------|------|----------|
| **System Prompt 层** | 所有 Agent 隐含"不提供诊断/治疗/处方"指令 | — |
| **输出后处理层** | `check_response_prohibited()` 扫描 18 个禁用词 | 临床术语 10 + 边界越权 8 |
...
| 级别 | 展示内容 | API |
|------|----------|-----|
| YELLOW | 自助资源：呼吸放松(4-4-6) · 接地技巧(5-4-3-2-1) · 情绪书写 · 身体照顾 · 蝴蝶拥抱 | `GET /api/safety/self-help` |
| ORANGE | 专业引导：找咨询师 5 步 · 首次咨询说明 · 壹心理/简单心理/KYS 平台 | `GET /api/safety/professional` |
| ORANGE/RED | 热线：400-161-9995(24h) · 010-82951332(24h) · 400-821-1215 + 6 城市机构 + 12338/12355 | `GET /api/safety/hotlines` |
...
| 组件 | 触发条件 | 持久化 |
|------|----------|--------|
| `ConsentModal` | 首次访问（无 `localStorage` 记录） | `lens_consent_accepted` |
| 测评引导弹窗 | 同意后 + 无测评记录 | `lens_assessment_prompted` |
| 永久免责条 | 每次访问 Chat/Arena | 页面内嵌 |

### 20.9 Arena 安全治理

Arena 路径完整接入 `CrisisDetector`：

| 行为 | 说明 |
|------|------|
| RED 中断 | 双路均返回危机模板，`requires_vote=false`，写入 `crisis_archive/`（`source: "arena"`） |
| YELLOW/ORANGE 注入 | 向 A/B 双方 system prompt 注入安全引导 |
| 禁用词后处理 | `_sanitize_with_crisis_guard()` 扫描 A/B 回复 |
| 前端 | RED 轮次显示红色标签，不弹打分面板 |

### 20.10 核心文件索引

| 文件 | 职责 |
|------|------|
| `scripts/advisor/api/crisis_detector.py` | `CrisisDetector` 类：五步检测 + 禁用词扫描 + 安全引导生成 |
| `configs/crisis_keywords.yaml` | 82 条触发词 + 11 条排除模式 + 18 条禁用词 |
| `configs/crisis_resources.yaml` | 热线(4全国+6地区+2特殊) + 自助(5模块) + 专业(3平台) + 响应模板(3级) |
| `configs/consent_text.yaml` | 知情同意 4 板块文本 |
| `frontend/src/components/safety/ConsentModal.tsx` | 知情同意弹窗 + 测评引导弹窗 |
| `frontend/src/components/safety/CrisisBanner.tsx` | 紧急求助按钮 |
| `advisor_out/crisis_archive/` | RED 级别事件归档（JSONL） |

---

## 21. Arena 双镜对比与 Elo 排名

> 📖 **详细设计文档**：[arena_dual_mirror_overview.md](arena_dual_mirror_overview.md) — 交互设计、Elo 算法实现、安全治理、数据结构、前端组件、高级分析全覆盖

Arena 系统在沉浸式互动的基础上并列两个聊天框，同时针对同一问题进行多轮对话，每轮输出完成后弹出打分面板。

### 21.1 核心交互流程

```
用户输入 → POST /api/arena/chat
  ├─ 危机检测（四级）→ RED 中断 / YELLOW·ORANGE 注入安全引导
  ├─ 构建 system prompt（Agent prompt + 安全引导 + 测评注入 + RAG + 记忆摘要 + 关键事实）
  ├─ asyncio.gather 双路并发生成
  ├─ 禁用词后处理
  └─ 返回 { response_a, response_b, crisis_level, requires_vote }
      │
      ▼
弹出打分面板 → 投票(5选1) + 五维评分(1-10×2) + 备注(240字) → POST /api/arena/vote
      │
      ├─ 写入 session 对应轮次
      ├─ 追加 battles.jsonl
      └─ 揭示 A/B 模型身份
...
| 能力 | 说明 | 实现 |
|------|------|------|
| **双路并发对话** | 同一输入 → A/B 模型 `asyncio.gather` 并发 | ✅ |
| **五维评分** | 共情 · 深度 · 实用 · 专业 · 流畅，每维度 1-10 滑块 | ✅ |
| **投票系统** | a_win / b_win / tie / both_good / both_bad | ✅ |
| **评分备注** | 可选文本框，持久化到 session + battles.jsonl | ✅ |
| **Bradley-Terry Elo** | MLE 全量重算（50 次迭代），主 Elo + 五维独立 Elo | ✅ |
| **多轮 session** | JSON 持久化，支持历史加载和侧边栏列表 | ✅ |
| **三种对比模式** | 模型对决 ✅ / 流派对比 ✅(5种流派) / 视角碰撞 ⬜(依赖S6) | ✅/⬜ |
| **模型揭示策略** | 投票后自动揭示 + Toggle 可开可关 + 揭示后模型切换器 | ✅ |
| **单路退化** | ≥1 轮后选择一侧，关闭另一侧，居中显示，打分保留 | ✅ |
| **安全治理** | 四级危机检测 + 禁用词后处理 + 危机归档 | ✅ |
| **长对话记忆** | 滑动窗口 12 轮 + 摘要压缩 + 关键事实(≤20条) + RAG(top-3) | ✅ |
| **偏好统计** | 各模型被选择次数和占比 | ✅ |
| **Query 分层** | 5 类自动分类 × 模型胜率 | ✅ |
| **标注质量** | am-ELO 五维方差分析，检测随意/极端打分 | ✅ |
| **对话导出** | JSON / Markdown 格式 | ✅ |
| **快速复用** | 沉浸式互动 → Arena 预填问题 | ✅ |
| **移动端适配** | < md 断点 Tab 切换 A/B | ✅ |

### 21.3 API 端点

| 方法 | 路径 | 功能 | 返回关键字段 |
|------|------|------|-------------|
| `POST` | `/api/arena/chat` | 多轮对话（危机检测 → 并发生成 → 禁用词后处理） | `arena_session_id, response_a, response_b, crisis_level, requires_vote` |
| `POST` | `/api/arena/vote` | 投票 + 五维评分 + 备注 | `contestant_a, contestant_b` |
| `GET` | `/api/arena/sessions` | 会话列表 | `[{ id, title, rounds, communication_status, time }]` |
| `GET` | `/api/arena/session/{id}` | 会话详情（含全部轮次） | 完整 session JSON |
| `GET` | `/api/arena/stats` | Elo 排名（惰性重算：对局数变化时触发） | `{ updated_at, total_battles, ratings }` |
| `GET` | `/api/arena/leaderboard` | 排行榜别名 | 同 stats |
| `GET` | `/api/arena/summary` | 偏好统计 | `{ total, preference: { model: count } }` |
| `GET` | `/api/arena/query-stats` | Query 分层统计（5 类 × 模型胜率） | `{ categories, total }` |
| `GET` | `/api/arena/annotator-stats` | am-ELO 标注质量 | `{ consistency_score, sessions_analyzed, details }` |

### 21.4 Bradley-Terry Elo 算法

**选型**：全量 MLE 重算（非增量 K-factor），参考 LMSYS Chatbot Arena，消除对局顺序依赖。

```python
def bt_mle(win_matrix, keys, n_iter=50):
    r = {k: 1000.0 for k in keys}       # 初始 1000
    for _ in range(n_iter):
        for i in keys:
            numer = Σ win_matrix[i][j]   # i 的总胜场
            denom = Σ total_ij × r[j] / (r[i] + r[j])
            r[i] = max(100, r[i] × numer / denom)
    scale = 1000 / mean(r)               # 归一化均值到 1000
...
|------|-----|------|
| 初始 Elo | 1000 | MLE 先验 |
| 迭代次数 | 50 | 通常 30-100 即收敛 |
| tie 处理 | 0.5-0.5 | 双方各计半胜 |
| 95% CI | `Elo ± 2 × 400/√n` | n=对局数 |
| 冷启动 | < 10 场 | 前端标记「未排名」 |
| 五维 Elo | 独立 MLE | `score_a[dim]` vs `score_b[dim]` 决定维度级胜负 |

### 21.5 五种顾问流派

| agent_type | 名称 | 理论框架 | 特色机制 |
|-----------|------|---------|---------|
| `neutral` | 中立顾问 | 多维分析（NVC/依附/权力动态） | — |
| `supportive` | 支持性顾问 | Rogers 无条件积极关注 | — |
| `psychoanalytic` | 精神分析顾问 | 客体关系 + 拉康三界 | — |
| `eft` | EFT 情绪聚焦 | Sue Johnson Tango 9 步 | 阶段推进（exploration→comforting→action）+ 安全阀 |
| `bowen` | 家庭系统顾问 | Bowen 八大概念 | 第三方检测 + 三角关系注入 + "假设+证据+验证"格式 |

新增顾问只需修改 3 文件：`core/prompts.py`（prompt）+ `types/index.ts`（类型）+ `constants.ts`（前端配置）。

### 21.6 五维评分体系

| 维度 | 键 | 评估焦点 | 范围 |
|------|-----|----------|------|
| 共情 | `empathy` | 情绪识别准确性、共情语言质量 | 1-10 |
| 深度 | `depth` | 理论引用、多维分析、模式识别 | 1-10 |
| 实用 | `practicality` | 行动步骤明确性、示例话术质量 | 1-10 |
| 专业 | `professionalism` | 边界意识、非诊断性表达、伦理合规 | 1-10 |
| 流畅 | `fluency` | 逻辑连贯性、阅读体验、表达精炼度 | 1-10 |
...
advisor_out/arena/
├── sessions/
│   └── arena-{uuid8}.json         # 会话：contestant_a/b + rounds[] + memory_facts[]
│       rounds[i]: { query, response_a, response_b, vote, scores{a,b}, remark, crisis_level, timestamp }
├── battles.jsonl                   # 对局日志（每轮一行 JSONL，含完整回复和评分）
└── elo_ratings.json                # Elo 排名快照（惰性重算后缓存）
    ratings[key]: { overall, ci_95, empathy..fluency, battles, wins, losses, ties, contestant }
```

### 21.8 高级分析

| Analysis | API | Method | Purpose |
|------|-----|------|------|
| **Preference Stats** | `GET /api/arena/summary` | Count total a_win/b_win per model | Default recommendation |
| **Query Stratification** | `GET /api/arena/query-stats` | 5 categories (Emotional/Conflict/Advice/Relation/General) × Win Rate | Edge case detection |
| **Annotation Quality** | `GET /api/arena/annotator-stats` | am-ELO consistency: 0.3-1.0 score | Weighting logic |
| **Arena Session Mgmt** | `/api/arena/sessions/{id}` | **PUT (Rename) / DELETE (Delete)** | Session management |
| **Chat Session Mgmt** | `/api/chat/sessions/{id}` | **PUT (Rename) / DELETE (Delete)** | Session management |

### 21.9 前端组件

| 文件 | 行数约 | 功能 |
|------|--------|------|
| `ArenaPage.tsx` | ~630 | 双路聊天 + 打分面板 + 侧边栏 + 导出 + 单路退化 + 移动端 Tab |
| `ArenaStatsPage.tsx` | ~240 | 排行榜(sortable) + 偏好统计柱状图 + Query 分层 + 标注质量 |
| `useArenaStore.ts` | ~105 | Zustand 全局状态（battleId, contestants, scores, revealed, mode） |
| `ExportDialog.tsx` | ~118 | JSON/Markdown 导出（`arenaToExportData` 格式化） |

### 21.10 下游数据消费

```
battles.jsonl
  ├─→ Elo MLE 重算 → elo_ratings.json → 排行榜展示
  ├─→ KTO/DPO 训练：a_win → chosen=A/rejected=B; b_win → 反之; tie → skip; both_bad → 双 reject
  ├─→ Query 分层统计 → 薄弱场景 → Prompt 针对性优化
  ├─→ am-ELO 标注质量 → 低质量打分降权
  └─→ 维度短板分析 → 某模型"深度"长期低 → 引入深度分析模块
```
...
> 📖 **详细设计文档**：[监督评估系统设计概览](supervision_pipeline_overview.md) — 包含 LLM-as-Judge 架构、六维度打分机制、优先级链降级策略、前端交流状态展示和数据持久化的完整说明。

### 22.1 核心架构

监督评估系统（Supervision Pipeline）是独立于主对话流的**质量保障与偏见防控模块**。每轮对话完成后，系统异步调用独立 Judge 模型（与对话模型不同），按六维度对 AI 回复进行结构化评估。
...
| 维度 | 键 | 分数方向 | 评估焦点 |
|------|-----|---------|---------|
| **对话进展** | `dialogue_progress` | 阶段标签 | 探索/安抚/行动阶段推进 |
| **权力动态** | `power_dynamics` | 1-10 (10=健康) | 福柯凝视：谁主导意义建构 |
| **共情与针对性** | `empathy_specificity` | 1-10 (10=精准) | 情绪识别准确性、建议贴合度 |
| **安全与边界** | `safety_boundary` | 1-10 (10=合规) | 越界检测、诊断性语言拦截 |
...
## 23. S6 跨学科理论引擎与视角碰撞模式

> **实现状态**：✅ 已完成（2026-03-08）
>
> **设计理念**：传统 AI 关系咨询局限于心理学单一视角。S6 引入社会学、哲学、博弈论/行为经济学、文化研究四大学科视角，让用户从不同透镜审视同一问题——不是"哪个视角更好"，而是"不同学科看到完全不同的面向"。
...
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │  core/prompts.py :: CHAT_SYSTEM_PROMPTS              │  │
│  │  ┌──────────┬──────────┬──────────┬──────────┐       │  │
│  │  │sociology │philosophy│game_theory│ cultural │       │  │
│  │  │listen    │listen    │listen     │listen    │       │  │
│  │  │consult   │consult   │consult    │consult   │       │  │
│  │  └──────────┴──────────┴──────────┴──────────┘       │  │
│  │  → 与 neutral/supportive/psychoanalytic/eft/bowen    │  │
│  │    同级并列，零耦合                                   │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │  advisor_out/knowledge/perspectives/                 │  │
│  │  ├── sociology.jsonl    (10 条 FAQ)                  │  │
│  │  ├── philosophy.jsonl   (10 条 FAQ)                  │  │
│  │  ├── game_theory.jsonl  (10 条 FAQ)                  │  │
│  │  └── cultural.jsonl     (10 条 FAQ)                  │  │
│  │  → _search_faq(agent_type) 自动按视角过滤           │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │  Arena 视角碰撞模式（ArenaPage.tsx）                  │  │
│  │  CompareMode = 'perspective'                         │  │
│  │  → PERSPECTIVE_IDS 过滤：仅 S6 四视角               │  │
│  │  → 流派对比仅心理学流派，互不混合                    │  │
│  │  → Elo 独立排名：mode=perspective                    │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │  远期：GraphRAG 跨学科知识图谱（§23.6）              │  │
│  │  NetworkX(原型) → Neo4j(生产)                        │  │
│  │  cross_discipline_link 关系实现概念间多跳推理         │  │
│  └───────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
| agent_type | 前端名称 | 核心理论框架 | 典型输出示例 | 知识库条目 |
|-----------|---------|-------------|------------|-----------|
| `sociology` | 社会学视角 | 布尔迪厄场域/资本/惯习 · 吉登斯纯粹关系与反思性监控 · 戈夫曼拟剧论(前台/后台) · 贝克风险社会 · 哈贝马斯沟通理性 | "你们的冲突反映了不同阶层'惯习'的碰撞..." | 10 条 |
| `philosophy` | 哲学视角 | 存在主义(萨特/海德格尔) · 现象学(梅洛-庞蒂身体主体性) · 实用主义(杜威) · 关怀伦理(诺丁斯) · 儒家自我修养 | "从存在主义角度，关系不是发现而是共同创造..." | 10 条 |
| `game_theory` | 博弈论视角 | 囚徒困境 · 纳什均衡 · 沉没成本/损失厌恶(行为经济学) · 锚定效应 · 信号理论 · 不完全契约 | "你们陷入了'囚徒困境'——各自理性但集体不优..." | 10 条 |
| `cultural` | 文化视角 | 差序格局(费孝通) · 集体/个人主义(Hofstede) · 面子/人情/报 · 过渡仪式(Van Gennep) · WEIRD偏差 · 深描(Geertz) | "在差序格局中，你的困境是两个伦理圈层的碰撞..." | 10 条 |

### 23.3 Prompt 设计原则

所有 4 个视角的 System Prompt 遵循统一结构：
...
│  2. 核心分析框架（3-5 个主要理论）       │
│  3. 输出规范                            │
│     - listen 模式：5-7 句共情+1 个探索   │
│     - consult 模式：800-1500 字多维分析  │
│  4. 通俗化要求                          │
│     - 每个概念 = 1 句话定义 + 生活例子   │
│     - 禁止堆砌学术术语                   │
│  5. [安全规则]                           │
...
每条 FAQ 为 JSONL 格式，字段如下：

| 字段 | 类型 | 说明 |
|------|------|------|
| `question` | string | 用户可能提出的问题 |
| `answer` | string | 包含理论概念的回答 |
| `category` | string | 学科分类（sociology/philosophy/game_theory/cultural） |
| `keywords` | string[] | 用于检索匹配的关键词 |
| `source` | string | 理论来源/学者 |
| `license` | string | 许可（public_domain/CC-BY-4.0） |

**示例**（sociology.jsonl）：
```json
{
  "question": "为什么我们的争吵总是围绕钱和教育观展开？",
  "answer": "布尔迪厄的「资本」理论可以解释这个现象：关系中的冲突往往不只是表面议题...",
  "category": "sociology",
  "keywords": ["布尔迪厄", "资本", "惯习", "阶层"],
...
| 要素 | 设计 | 文件 |
|------|------|------|
| **CompareMode** | `'model'` / `'agent_type'` / `'perspective'` | `ArenaPage.tsx` |
| **视角过滤** | `PERSPECTIVE_IDS = Set(['sociology','philosophy','game_theory','cultural'])` | `ArenaPage.tsx` |
| **流派过滤** | `pickerPersonas = PERSONAS.filter(p => !PERSPECTIVE_IDS.has(p.id))` | `ArenaPage.tsx` |
| **自动选择** | 切换到视角碰撞时自动设为 社会学 vs 哲学 | `ArenaPage.tsx` |
| **标签** | 视角 A / 视角 B · 匿名视角 · 揭示后显示视角名 | `ArenaPage.tsx` |
| **Elo Tab** | `STATS_MODES` 增加 `{ value: 'perspective', label: '视角碰撞' }` | `ArenaStatsPage.tsx` |

#### 后端（零改动）

Arena 后端通过 `CHAT_SYSTEM_PROMPTS.get(agent_type)` 自动适配所有 9 个 agent_type，无需任何 server.py 修改。`mode=perspective` 的对局在 `battles.jsonl` 中独立记录，Elo MLE 按 mode 分桶重算。

### 23.6 远期：GraphRAG 跨学科知识图谱

> **触发条件**：知识库 FAQ 条目 > 200 条后，简单向量检索无法捕捉跨学科概念关联。

#### 为什么需要 GraphRAG？

当前 FAQ 检索是平面向量匹配——搜索「追逃循环」只能找到 EFT 知识。但「追逃循环」与博弈论的「囚徒困境」、社会学的「权力不对称」存在深层关联。GraphRAG 通过知识图谱实现这种跨学科多跳推理。

#### 双栈渐进策略

| 阶段 | 技术栈 | 规模 | 时机 |
|------|--------|------|------|
| **原型期** | NetworkX (Python 内存图) | < 500 节点 | FAQ > 200 条 |
| **生产期** | Neo4j (图数据库) + Cypher 查询 | > 500 节点 | 用户量扩展后 |
...
关系类型 (9)：
  ...
  game_with      - 博弈
  exchanges      - 交换
  cross_discipline_link - 跨学科关联 ← 核心差异化
```

#### 与现有 RAG 的集成路径

```python
def _graph_enhanced_rag(query: str, perspective: str) -> str:
    """图增强 RAG：向量检索 + 图谱多跳推理"""
    # 1. 现有向量检索（FAISS top-10）
    vector_results = existing_rag.search(query)
    
    # 2. 图谱查询：从 query 概念出发，2-hop 邻域
    graph_results = knowledge_graph.query(
        start_concepts=extract_concepts(query),
        max_hops=2,
        perspective_filter=perspective  # 只返回相关学科
    )
    
    # 3. 融合：向量结果 + 图谱关系 → 注入 _build_rag_context()
    return merge_contexts(vector_results, graph_results)
```

**集成点**：`_build_rag_context()` 的 `[专业知识]` 段，对现有代码零侵入。

#### 跨学科关联示例

```
追逃循环(EFT) ←cross_discipline→ 囚徒困境(博弈论)
    "两人都想亲密但又害怕受伤 → 各自理性但集体不优"

场域竞争(社会学) ←cross_discipline→ 纳什均衡(博弈论)
    "职场权力博弈的结构性分析"

差序格局(文化) ←cross_discipline→ 关怀伦理(哲学)
    "近远亲疏的伦理张力 → 关怀的圈层与扩展"
```
...
| 文件 | 职责 |
|------|------|
| `scripts/advisor/api/core/prompts.py` | 4 个视角 × 2 模式 = 8 个 System Prompt |
| `advisor_out/knowledge/perspectives/sociology.jsonl` | 社会学 FAQ（10 条） |
| `advisor_out/knowledge/perspectives/philosophy.jsonl` | 哲学 FAQ（10 条） |
| `advisor_out/knowledge/perspectives/game_theory.jsonl` | 博弈论 FAQ（10 条） |
| `advisor_out/knowledge/perspectives/cultural.jsonl` | 文化 FAQ（10 条） |
| `frontend/src/constants.ts` | 4 个新 Persona 定义（颜色/图标/描述） |
| `frontend/src/types/index.ts` | PersonaType 联合类型扩展 |
| `frontend/src/pages/ArenaPage.tsx` | 视角碰撞 CompareMode + PERSPECTIVE_IDS 过滤 |
| `frontend/src/pages/ArenaStatsPage.tsx` | 视角碰撞 Elo Tab |
