...
vim /data/CHAT_APP_DHA/ls_windsurf/local_secrets/.env.advisor

# 2. 加载到环境
source /data/CHAT_APP_DHA/ls_windsurf/local_secrets/.env.advisor
```
...
export ANTHROPIC_API_KEY="sk-ant-..."  # DeepSeek 审核用
export OPENAI_API_KEY="sk-..."        # 可选
# ... 其他后端按需配置
```

> **注意**: 你不需要配置所有 API Key。推荐至少配置 **DeepSeek**（生成）+ **DeepSeek**（审核）。
...
|------|----------|----------|----------|
| `openai` | GLM-4.7 | 第三方代理 | analysis, review, chat |
| `DeepSeek` | DeepSeek-V3.2 | 第三方代理 | analysis, review, chat |
| `Kimi` | Kimi-K2.5 | Google AI Studio | analysis, review, chat |
| `kimi` | kimi-k2.5 | https://api.moonshot.cn/v1 | analysis, chat |
| `Qwen` | Qwen3 | https://api.x.ai/v1 | analysis, review, chat |
...
python scripts/advisor/run_all/_02_generate_analysis.py \
    --backend Qwen --agent-type neutral

# 三种 Agent 都生成
for t in neutral supportive psychoanalytic; do
    python scripts/advisor/run_all/_02_generate_analysis.py --backend Qwen --agent-type $t
done

# 测试模式（只处理前 5 个片段）
python scripts/advisor/run_all/_02_generate_analysis.py \
    --backend Qwen --agent-type neutral --limit 5
```
...
> Phase 2 通过 API 运行时，DeepSeek 后端默认使用 `DeepSeek-V3.2`。
...
```json
{
  ...
  "messages": [
    ...
    {"role": "assistant", "content": "...", "timestamp": "...", "backend": "Qwen", "model": "Qwen3", "timestamp": "..."}
  ],
  "created_at": "...",
  "updated_at": "..."
}
...
│  Phase 1: done ✅  │  Qwen: connected ✅    │
│  Phase 2: idle     │  DeepSeek: connected ✅   │
...
| 平台 | 可用模型 |
|------|----------|
| **第三方代理** (key1 DeepSeek专用) | DeepSeek-V3.2 ✅, DeepSeek-V3.2-Speciale ✅, DeepSeek-Coder-V2 ✅ |
| **第三方代理** (key2/3 通用) | DeepSeek-V3.2 ✅, Qwen3 ✅, Qwen3-235B ✅, GLM-4.7 ✅, Kimi-K2.5 ✅ |
| **第三方代理** | GLM-4.7 ✅, DeepSeek-V3.2 ✅, Kimi-K2.5 ✅, Qwen3 ✅ |
| **第三方代理** | DeepSeek-V3.2 ✅, Qwen3 ✅, Qwen3-fast ✅ |
| **本地** | Ollama qwen3:8b ✅ |
...
| 场景 | 推荐 | 原因 |
|------|------|------|
| **日常生成** | DeepSeek V3.2 / Qwen3 | 性价比高，思考链强 |
| **质量审核** | DeepSeek V3.2 | 推理深度最好 |
| **精神分析** | DeepSeek V3.2 | 复杂心理分析洞察力 |
| **实时对话** | Qwen3 | 流式快，质量高 |
...
**推荐**: DeepSeek V3 生成 + DeepSeek V3 审核 ≈ **$6.40 总计**
...
思考模型（如 Qwen3）的 `</think>` 推理过程消耗大量 token。后端已自动处理：检测到模型名含 `think` 时不设置 `max_tokens`，让模型用满上下文窗口。
...
**DeepSeek V3.2** 性价比最高。对话推荐 **Qwen3**（流式快、质量高）。
