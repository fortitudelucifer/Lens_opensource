"""
test_sticker_schema.py
Sticker 模态 merged_final.jsonl 输出格式的属性测试
...
运行方式：
    python -m pytest tests/test_sticker_schema.py -v
"""
...
from scripts._common.path_utils import get_sticker_after_merge


def load_sticker_merged_final():
    """加载 sticker_merged_final.jsonl 文件"""
    after_merge = get_sticker_after_merge()
    output_file = after_merge / 'sticker_merged_final.jsonl'
...
class TestStickerSchemaProperties:
    """Sticker 模态 Schema 属性测试"""
    
    @pytest.fixture(scope="class")
    def records(self):
        """加载测试数据"""
        return load_sticker_merged_final()
        ...
        **Validates: Requirements 5.1, 5.2**
        ...
        for i, record in enumerate(records):
            ...
            if common_indices:
                ...
                for field in STICKER_SPECIFIC_FIELDS:
                    if field in keys:
    ...
    def test_modality_value(self, records):
        """测试 modality 值为 'sticker'"""
        for i, record in enumerate(records):
            assert record.get('modality') == 'sticker', \
                f"记录 {i}: modality 应为 'sticker'，实际为 '{record.get('modality')}'"
